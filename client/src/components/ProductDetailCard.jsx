...
const ProductDetailCard = () => {
  ...
  if (!current_product) {
    return <div>Loading...</div>; // Placeholder while the product is being fetched
  }

  return (
    <>
    <div onClick={() => navigate("/")} className="text-blue-500 text-center underline underline-offset-4 cursor-pointer my-2 ">Go&nbsp;back&nbsp;to&nbsp;products</div>
                  ...
                  {reviews.length > 0 && reviews.map((review, index) => (
                    ...
                    >
                      <div className="flex items-center">
                        <FaUserCircle />
                        <span className="font-bold ml-1"> {review.userId.username}:</span> 
                      </div>
                      <p>{review.comment} --- <span className="text-sm">sentiment:{review.result.label}</span></p>
                    </li>
                  ))}
                </ul>
              ) : (
